...
/*  Run2 include */
#include "microAODProduction/interface/Htautau_2015.h"
#include "microAODProduction/interface/SyncTree.h"
#include "AnalysisBase/src/BTagCalibrationStandalone.cc"
...
struct jetEffInfo{
  ...
  jetEffInfo(const Run2::Sync& event, int jetIndex):eff(0.),SF(0.){
...
public:
  BjetEffSF(const std::string& inputFileName, const std::string& outputFileName,
            const std::string& bTagEffName = "Analysis/data/bTagEff_Loose.root", const std::string& bjetSFName = "Analysis/data/CSVv2.csv")
  :inputFile(root_ext::OpenRootFile(inputFileName)),
   ...
   syncTree(new Run2::SyncTree("sync", inputFile.get(), true)),anaData(outputFile){
  ...
  void Run(){
    for(Long64_t current_entry = 0; current_entry < syncTree->GetEntries(); ++current_entry) {
        ...
        const Run2::Sync& event = syncTree->data;
        ...
        for (int jetIndex = 0; jetIndex < event.njets; jetIndex++){
          ...
          if      (abs (jetInfo.partonFlavour) == 5 ) {
            jetInfo.SF  = reader->eval(BTagEntry::FLAV_B, jetInfo.eta, jetInfo.pt);
            jetInfo.eff = GetEfficiency(eff_b,jetInfo.pt,TMath::Abs(jetInfo.eta));
          }
          else if (abs (jetInfo.partonFlavour) == 4 ) {
            jetInfo.SF = reader->eval(BTagEntry::FLAV_C, jetInfo.eta, jetInfo.pt);
            jetInfo.eff = GetEfficiency(eff_c,jetInfo.pt,TMath::Abs(jetInfo.eta));
          }
          else {
            jetInfo.SF = reader_light->eval(BTagEntry::FLAV_UDSG, jetInfo.eta, jetInfo.pt);
...
private:
  ...
  std::shared_ptr<Run2::SyncTree> syncTree;
  BjetStudyData anaData;
  BTagCalibration *calib;
  BTagCalibrationReader *reader, *reader_light;
