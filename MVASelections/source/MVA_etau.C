...
GetMVA(){
    ...
    TFile *inputSignal  = TFile::Open("/Users/rosmina/Desktop/FLAT_TREE/etau/ggH_tot.root");
    TFile *inputBkg1    = TFile::Open("/Users/rosmina/Desktop/FLAT_TREE/etau/tt_tot.root");
	...
	for (UInt_t i=0; i<(sigTree->GetEntries()); i++) {
		sigTree->GetEntry(i);
		if( (byCombinedIsolationDeltaBetaCorrRaw3Hits2<1.5) && (pfRelIso1<0.1) && (Q1*Q2<0) && (mt1<30)){
			//if(!(nBjet>1)) continue;
			vars[2]=0;
			vars[3]=0;
			...
			if ((IndexNjetEta2p4.size() > 1)) {
				...
				if ((BjetsCut == true)){
					...
					else {cout<<"Riempi Test ev="<<i<<endl;
